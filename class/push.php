...
class Push {
	...
	public function get_settings() {
		...
		$sql = "SELECT * FROM `push_push`.`sites` WHERE `sub` = '$server[0]'";
	...
	public function manage() {
		...
		$sql = "SELECT * FROM `push_push`.`sites` ORDER BY `sub` ASC";
		$result = $this->new_mysql($sql);
		while ($row = $result->fetch_assoc()) {
			$i++;
			if ($i % 2) {
	...
	public function edit_site() {
		...
		$sql = "SELECT * FROM `push_push`.`sites` WHERE `id` = '$_GET[id]'";
		$result = $this->new_mysql($sql);
		while ($row = $result->fetch_assoc()) {
	...
	public function delete_site() {
		...
		$sql = "DELETE FROM `push_push`.`sites` WHERE `id` = '$_GET[id]'";
		$result = $this->new_mysql($sql);
		if ($result == "TRUE") {
	...
	public function update_site() {
		...
		$sql = "UPDATE `push_push`.`sites` SET `app_id` = '$_POST[app_id]', `crypto` = '$_POST[crypto]', `uupass` = '$_POST[uupass]' $pem $logo WHERE `id` = '$_POST[id]'";
		$result = $this->new_mysql($sql);
		if ($result == "TRUE") {
	...
	public function create_new_site() {
		...
		// check for username
		$sql = "SELECT `uuname` FROM `push_push`.`sites` WHERE `uuname` = '$_POST[uuname]'";
		$result = $this->new_mysql($sql);
		while ($row = $result->fetch_assoc()) {
		...
		// check for sub
                $sql = "SELECT `sub` FROM `push_push`.`sites` WHERE `sub` = '$_POST[sub]'";
                $result = $this->new_mysql($sql);
                while ($row = $result->fetch_assoc()) {
		...
		$domain_pw = $server_settings[2];
			...
			$sql = "INSERT INTO `push_push`.`sites` (`sub`,`pem`,`app_id`,`crypto`,`logo`,`uuname`,`uupass`) VALUES
